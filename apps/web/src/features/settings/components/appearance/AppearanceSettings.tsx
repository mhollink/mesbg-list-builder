...
export function AppearanceSettings() {
  ...
  const appPresets = useMemo(
    () =>
      allPresets.filter(
        ({ category }) => category === "middle-earth" || category === "default",
      ),
    [],
  );
  const creatorPresets = useMemo(
    () => allPresets.filter(({ category }) => category === "creator"),
    [],
  );
  const patreonPresets = useMemo(
    () => allPresets.filter(({ category }) => category === "patreon"),
    [],
  );
  ...
  return (
    <Stack spacing={4} sx={{ pb: 4 }}>
      ...
      <Box>
        ...
        >
          <ToggleButton value="light" aria-label={t("appearance.mode.light")}>
            <LightModeRoundedIcon sx={{ mr: 1 }} />
            {t("appearance.mode.light")}
          </ToggleButton>

          <ToggleButton value="dark" aria-label={t("appearance.mode.dark")}>
            <DarkModeRoundedIcon sx={{ mr: 1 }} />
            {t("appearance.mode.dark")}
          </ToggleButton>

          <ToggleButton value="system" aria-label={t("appearance.mode.system")}>
      ...
      <Box>
        ...
        >
          ...
          <CustomThemeCard
            ...
            primaryColor={
              selection.type === "custom" ? selection.primaryColor : "#1976D2"
            }
            onChange={(color) => dispatch(setCustomPrimaryColor(color))}
          />
        </Box>
